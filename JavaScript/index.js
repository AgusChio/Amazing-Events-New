...
createApp({
    ...
    computed: {
        filters(){
            let firstFilterCheckBox = this.backupEvents.filter( event => this.categoriesChecked.includes(event.category) || this.categoriesChecked.length === 0);
            ...
            this.events = secondFilterSearch;
        }
    }
}).mount('#app')
